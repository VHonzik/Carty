...
class CanBeInHandUpdate : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        _card.transform.position = VisualBridge.Instance.HandPositioning.PositionPlayer(0, 1);
        _card.transform.rotation = VisualBridge.Instance.HandPositioning.RotationPlayer(0, 1);
        _hand.IsInHand = true;
    ...
    void Update()
    {
        if (UpdateTime >= 2.0f)
        {
            Vector3 wanted = VisualBridge.Instance.HandPositioning.PositionPlayer(1, 2);
            IntegrationTest.Assert(_card.transform.position == wanted);

            var wanted_rot = VisualBridge.Instance.HandPositioning.RotationPlayer(1, 2);
