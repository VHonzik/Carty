...
namespace CartyVisuals
{
    ...
    class DefaultCardPositionInHand : IHandCardPositioning
    {
        ...
        private Quaternion Rotation(int index, int cards, bool player)
        {
            ...
            {
                return player ? VisualBridge.Instance.FlippedOn : VisualBridge.Instance.FlippedOff;
            }

            float how_full = Mathf.Clamp((float)cards / (float)VisualBridge.Instance.MaxCardsInHand, 0, 1);
            ...
            Quaternion result = (player ? VisualBridge.Instance.FlippedOn : VisualBridge.Instance.FlippedOff)
        ...
        private Vector3 Position(int index, int cards, bool player)
        {
            ...
            if (!even_cards && (index == (cards / 2)))
            {
                return VisualBridge.Instance.PlayerHandPosition;
            }
            ...
            result -= middle_card;
            result.y = (player ? sign : -sign) * VisualBridge.Instance.CardHeight * (index_relative_to_center + 1);
            result.z *= (player ? 1 : -1);
